...
public class Decoder {
    ...
    String viterbi( String s ) {
		...
		// Our Viterbi-algorithm
    	System.err.println(s);

    	for (int t = 1; t < v.length; t++) {
    		for (int state = 0; state < RandomKey.NUMBER_OF_CHARS; state++) {
    			for (int transition = 0; transition < RandomKey.NUMBER_OF_CHARS; transition++) {
    				double newscore = a[transition][state] + b[state][index[t]] + v[t-1][transition];
    				if ( v[t][state] == 0 || newscore > v[t][state] ) {
    					v[t][state] = newscore;
    					backptr[t][state] = transition;
    				}
    			}
    		}
    	}
